...
public class ServiceIndicationMessage extends WSPPart implements IWapMessage {
    ...
    private WapPushSIMessageDescriptor indicatorDescriptor;

    public ServiceIndicationMessage(WapPushSIMessageDescriptor indicatorDescriptor) {
    ...
    public void encode(ByteBuffer byteBuffer) throws Exception {
        ...
        if (!CommonUtils.checkStringIsEmpty(this.indicatorDescriptor.getHrefUrl())) {
            URLEncoder urlEncoder = new URLEncoder(this.indicatorDescriptor.getHrefUrl(), WAPPushCommon.WBXML_SI_BASE);
            urlEncoder.decode();
