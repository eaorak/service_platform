...
public class ServiceLoadingMessage extends WSPPart implements IWapMessage {
    ...
    private WapPushSLMessageDescriptor indicatorDescriptor;

    public ServiceLoadingMessage(WapPushSLMessageDescriptor indicatorDescriptor) {
    ...
    public void encode(ByteBuffer byteBuffer) throws Exception {
        ...
        if (!CommonUtils.checkStringIsEmpty(this.indicatorDescriptor.getHrefUrl())) {
            URLEncoder urlEncoder = new URLEncoder(this.indicatorDescriptor.getHrefUrl(), WAPPushCommon.WBXML_SL_BASE);
            urlEncoder.decode();
