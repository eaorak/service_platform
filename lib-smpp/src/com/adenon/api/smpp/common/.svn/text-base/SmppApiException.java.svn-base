...
public class SmppApiException extends Exception {
    ...
    public SmppApiException(Exception exception) {
        super(exception);
        this.errorCode = FATAL_ERROR;
        this.errorDescription = exception.getMessage();
    }

    @Override
    public String getMessage() {
        int errorIndex = this.getErrorCode() & 0x0f;
        String retVal = "ERROR MESSAGE";
        try {
            retVal = errorCodeDescriptions[errorIndex] + " - " + this.getErrorDescription();
        } catch (Exception e) {
        }
        return retVal;
    }
